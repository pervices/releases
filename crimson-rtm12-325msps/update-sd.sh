...
if [ -z "$1" ]
  then
    sh create_ramdisk.sh; sh update-journal-size.sh; sh update-fs.sh; sh update-website.sh; sh update-mcu.sh; sh update-server.sh; sh update-fpga-sd.sh; echo "Waiting for server to come up"; sleep 30; sync; sh checkVersion.sh; sh adf-lut.sh; sleep 10800; sync && reboot;
  elif [ "$1" = "nolut" ]; then
